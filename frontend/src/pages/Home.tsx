...
const Home: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      // Load doctors from backend API (pass userId for per-user favorites)
      try {
      ...
      try {
        if (currentUser?.id) {
          ...
          if (cancelled) return
          // Transform backend appointments to frontend format, preserving doctor info
          ...
          setAppointments(frontendAppointments)
        }
      } catch (error) {
            ...
            {upcomingAppointments.length > 0 && (
              <div className="space-y-2 border-t pt-4">
                ...
                {(selectedDate 
                  ...
                  const doctorName = doctor?.name || apt.doctorName || 'Unknown Doctor'
                  const doctorPhoto = doctor?.photoUrl || apt.doctorPhoto || ''
                  const aptDate = new Date(apt.date)
                  return (
                    <div key={apt.id} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
                      <div className="flex items-center space-x-2">
                        <Avatar src={doctorPhoto} name={doctorName} size="sm" />
                        <div>
