...
const Payments: React.FC = () => {
    ...
    const loadPaymentMethods = async () => {
      ...
      if (userId) {
        try {
          ...
          const backendMethods = await paymentApi.getPaymentMethods(userId)
          // Transform backend methods to frontend format
          const frontendMethods = backendMethods.map((method) => ({
            id: method.id,
            type: method.type,
          ...
          setPaymentMethods(frontendMethods)
          localStorage.setItem(`MEDI_PAYMENT_METHODS_${userId}`, JSON.stringify(frontendMethods))
          // Set default payment method as selected
          const defaultMethod = frontendMethods.find((m: PaymentMethod) => m.isDefault)
          if (defaultMethod) {
            setSelectedPaymentMethod(defaultMethod.id)
          } else if (frontendMethods.length > 0) {
            setSelectedPaymentMethod(frontendMethods[0].id)
          }
        } catch (error) {
          console.error('Failed to load payment methods from backend:', error)
          // Fallback to localStorage (user-scoped) or sample data
          const storedPayments = localStorage.getItem(`MEDI_PAYMENT_METHODS_${userId}`)
          if (storedPayments) {
            ...
            localStorage.setItem(`MEDI_PAYMENT_METHODS_${userId}`, JSON.stringify(samplePaymentMethods))
        ...
        // No user logged in — don't load any payment methods
        setPaymentMethods([])
      }
    }
  ...
  const handleExpiryDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Remove existing slashes to work with clean digits
    const digits = value.replace(/\//g, '')

    // Limit to 4 digits (MMYY)
    const trimmed = digits.slice(0, 4)

    // Auto-insert "/" after the 2nd digit
    if (trimmed.length >= 3) {
      value = trimmed.slice(0, 2) + '/' + trimmed.slice(2)
    } else {
      value = trimmed
    }
  ...
  const handleAddCard = async () => {
    ...
    try {
      ...
      const backendMethod = await paymentApi.createPaymentMethod(userId, {
        type: 'credit',
      ...
      // Create local payment method object with backend ID
      ...
      localStorage.setItem(`MEDI_PAYMENT_METHODS_${userId}`, JSON.stringify(updatedMethods))
      setShowAddForm(false)
      setFormData({ cardHolder: '', cardNumber: '', expiryDate: '', cvv: '' })
    } catch (error) {
      console.error('Failed to save payment method to backend:', error)
  ...
  const handleDeleteCard = async (cardId: string) => {
    ...
    // Update local state and localStorage (user-scoped)
