...
import { sampleDoctors, sampleServices } from '../data/seed'
...
const Doctors: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const storedServices = localStorage.getItem('MEDI_SERVICES')
    if (storedServices) {
      setServices(JSON.parse(storedServices))
    } else {
      setServices(sampleServices)
      localStorage.setItem('MEDI_SERVICES', JSON.stringify(sampleServices))
    }
  }, [])
  ...
  const toggleServiceFavorite = (serviceId: string) => {
    const updatedServices = services.map(svc =>
      svc.id === serviceId ? { ...svc, isFavorite: !svc.isFavorite } : svc
    )
    setServices(updatedServices)
    localStorage.setItem('MEDI_SERVICES', JSON.stringify(updatedServices))
  }
